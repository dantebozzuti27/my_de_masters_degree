# 6-MONTH DATA ENGINEER CURRICULUM
## Goal: Land $160-170k Role by July 2026

**Schedule:** 25 hours/week
**Duration:** 24 weeks (168 days)
**Start Date:** February 2026
**Target End Date:** July 2026

---

## Current Progress

**21/168 days complete**

**Next:** Day 22 - Docker Compose + Local Dev Stack

---

## MONTH 1: FOUNDATIONS

**Project:** Credit Markets Pipeline
**Stack:** Python, AWS Lambda, S3, PostgreSQL, Airflow, Streamlit

### Week 1: Python Fundamentals

| Day | Topic | Status |
|-----|-------|--------|
| 1 | Development Environment Setup | Done |
| 2 | Python Variables & Syntax | Done |
| 3 | Functions & Control Flow | Done |
| 4 | Data Structures (Lists, Dicts) | Done |
| 5 | List Comprehensions | Done |
| 6 | Dictionaries & JSON | Done |
| 7 | File I/O & Context Managers | Done |

### Week 2: Production Python + Git

| Day | Topic | Status |
|-----|-------|--------|
| 8 | Error Handling & Defensive Code | Done |
| 9 | Python Classes & OOP | Done |
| 10 | Logging & Configuration | Done |
| 11 | Git Fundamentals | Done |
| 12 | GitHub PRs & Workflows | Done |
| 13 | Week 2 Project: CLI Data Tool | Done |
| 14 | Week 2 Review | Done |

### Week 3: AWS + Docker Basics

| Day | Topic | Status |
|-----|-------|--------|
| 15 | Advanced Git + Code Review | Done |
| 16 | AWS Account Setup (IAM, CLI) | Done |
| 17 | S3 Fundamentals | Done |
| 18 | Docker Fundamentals | Done |
| 19 | Project 1: Credit Markets Pipeline | Done |
| 20 | Project 1: Credit Markets Pipeline | Done |
| 21 | Project 1: Credit Markets Pipeline | Done |

### Week 4: Project 1 Part 1

| Day | Topic | Status |
|-----|-------|--------|
| 22 | Docker Compose + Local Dev Stack | Next |
| 23 | FRED + SEC EDGAR API Integration |  |
| 24 | AWS Lambda Deployment |  |
| 25 | S3 Storage + Data Partitioning |  |
| 26 | PostgreSQL Setup + Schema Design |  |
| 27 | Project 1 Deep Work |  |
| 28 | Review + Debug |  |

### Week 5: Project 1 Part 2

| Day | Topic | Status |
|-----|-------|--------|
| 29 | Data Transformation Pipeline |  |
| 30 | Airflow DAG Basics |  |
| 31 | Connect Airflow to Lambda + S3 |  |
| 32 | Streamlit Dashboard Basics |  |
| 33 | Dashboard Data Integration |  |
| 34 | Project 1 Polish + Documentation |  |
| 35 | Month 1 Review |  |

---

## MONTH 2: DBT MASTERY

**Project:** MLB Analytics Platform
**Stack:** Python, Snowflake, dbt, Streamlit

**Certification:** dbt Analytics Engineering (Day 62)

### Week 6: dbt Fundamentals

| Day | Topic | Status |
|-----|-------|--------|
| 36 | dbt Fundamentals (models, refs) |  |
| 37 | dbt Materializations |  |
| 38 | Staging → Intermediate → Marts |  |
| 39 | dbt Sources and Seeds |  |
| 40 | dbt Practice: Transform Stock Data |  |
| 41 | dbt Deep Dive |  |
| 42 | dbt Review + Testing Intro |  |

### Week 7: dbt Testing & Macros

| Day | Topic | Status |
|-----|-------|--------|
| 43 | dbt Testing (schema + data tests) |  |
| 44 | Custom dbt Tests |  |
| 45 | dbt Macros and Jinja |  |
| 46 | dbt Packages + Documentation |  |
| 47 | dbt Advanced Patterns |  |
| 48 | dbt Mastery Practice |  |
| 49 | dbt Cert Prep Start |  |

### Week 8: Project 2 Part 1

| Day | Topic | Status |
|-----|-------|--------|
| 50 | MLB Stats API Data Ingestion |  |
| 51 | Snowflake Setup |  |
| 52 | dbt Staging Models for MLB |  |
| 53 | dbt Intermediate Models |  |
| 54 | dbt Marts + Dimensional Modeling |  |
| 55 | Project 2 Deep Work |  |
| 56 | Project 2 Testing |  |

### Week 9: Project 2 Complete

| Day | Topic | Status |
|-----|-------|--------|
| 57 | dbt Test Coverage + Docs |  |
| 58 | Streamlit Dashboard for MLB |  |
| 59 | Dashboard Advanced Features |  |
| 60 | Project 2 Polish |  |
| 61 | dbt Certification Final Prep |  |
| 62 | dbt Cert Exam |  |
| 63 | Month 2 Review |  |

---

## MONTH 3: AIRFLOW + APPLYING

**Project:** Data Quality Platform
**Stack:** Python, Airflow, Great Expectations, PostgreSQL

### Week 10: Airflow Fundamentals

| Day | Topic | Status |
|-----|-------|--------|
| 64 | Airflow Architecture Deep Dive |  |
| 65 | DAG Design Patterns |  |
| 66 | Operators and TaskFlow API |  |
| 67 | Task Dependencies + XComs |  |
| 68 | Airflow Connections + Hooks |  |
| 69 | Airflow Lab: Complex DAG |  |
| 70 | Airflow Testing Intro |  |

### Week 11: Airflow Production + Apply

| Day | Topic | Status |
|-----|-------|--------|
| 71 | Airflow Testing + Monitoring |  |
| 72 | Error Handling in DAGs |  |
| 73 | Resume + LinkedIn Polish - START APPLYING |  |
| 74 | First 5 Applications |  |
| 75 | More Applications + Airflow Practice |  |
| 76 | Application Sprint (10 apps) |  |
| 77 | Week Review + Tracking |  |

### Week 12: Project 3 Part 1

| Day | Topic | Status |
|-----|-------|--------|
| 78 | Multi-Source Ingestion DAGs |  |
| 79 | Great Expectations Setup |  |
| 80 | Data Quality Framework |  |
| 81 | Quality Checks Integration |  |
| 82 | Applications + Project Work |  |
| 83 | Project 3 Deep Work |  |
| 84 | Project 3 Debug + Review |  |

### Week 13: Project 3 Complete

| Day | Topic | Status |
|-----|-------|--------|
| 85 | Monitoring + Alerting Setup |  |
| 86 | Final Polish + Deployment |  |
| 87 | Project 3 Documentation |  |
| 88 | Applications (25+ total) |  |
| 89 | More Applications |  |
| 90 | Application Sprint + Portfolio |  |
| 91 | Month 3 Review (30+ apps) |  |

---

## MONTH 4: AWS CERTIFICATION

**Certification:** AWS Solutions Architect Associate (Day 111)

### Week 14: AWS SAA - Compute & Storage

| Day | Topic | Status |
|-----|-------|--------|
| 92 | AWS S3 Deep Dive |  |
| 93 | AWS EC2 + Lambda |  |
| 94 | AWS RDS + Redshift |  |
| 95 | Applications + AWS Practice |  |
| 96 | AWS EBS Storage |  |
| 97 | AWS Lab Day |  |
| 98 | AWS Review + Flashcards |  |

### Week 15: AWS SAA - Networking

| Day | Topic | Status |
|-----|-------|--------|
| 99 | AWS VPC Networking |  |
| 100 | AWS IAM Deep Dive |  |
| 101 | AWS Glue + Athena |  |
| 102 | Applications + Interview Prep |  |
| 103 | AWS CloudWatch |  |
| 104 | AWS Practice Exam 1 |  |
| 105 | Review Weak Areas |  |

### Week 16: AWS SAA - Final Prep

| Day | Topic | Status |
|-----|-------|--------|
| 106 | AWS Practice Exam 2 |  |
| 107 | AWS Weak Areas |  |
| 108 | AWS Practice Exam 3 |  |
| 109 | First Interviews |  |
| 110 | AWS Final Review |  |
| 111 | AWS Exam Day |  |
| 112 | Celebrate + Interview Prep |  |

### Week 17: Interview Ramp Up

| Day | Topic | Status |
|-----|-------|--------|
| 113 | Interview SQL |  |
| 114 | Interview Python |  |
| 115 | Interview System Design |  |
| 116 | Active Interviewing 1 |  |
| 117 | Interview + Applications |  |
| 118 | Mock Interview Day |  |
| 119 | Month 4 Review |  |

---

## MONTH 5: INTERVIEW INTENSIVE

### Week 18: System Design Practice

| Day | Topic | Status |
|-----|-------|--------|
| 120 | System Design: Data Pipelines |  |
| 121 | System Design: Data Warehouses |  |
| 122 | System Design: Streaming |  |
| 123 | Active Interviewing 2 |  |
| 124 | System Design: Realtime |  |
| 125 | System Design Practice |  |
| 126 | Interview Debrief |  |

### Week 19: Technical Interview Practice

| Day | Topic | Status |
|-----|-------|--------|
| 127 | SQL Window Functions |  |
| 128 | SQL CTEs + Subqueries |  |
| 129 | Python Data Structures |  |
| 130 | Active Interviewing 3 |  |
| 131 | Data Modeling Questions |  |
| 132 | Mock Interview Full |  |
| 133 | Interview Prep Review |  |

### Week 20: Behavioral + Mock Interviews

| Day | Topic | Status |
|-----|-------|--------|
| 134 | Behavioral STAR Method |  |
| 135 | Tell Me About Yourself |  |
| 136 | Leadership + Conflict |  |
| 137 | Active Interviewing 4 |  |
| 138 | Mock Interview 1 |  |
| 139 | Mock Interview 2 |  |
| 140 | Refine Answers |  |

### Week 21: Interview Refinement

| Day | Topic | Status |
|-----|-------|--------|
| 141 | Refine Story |  |
| 142 | Technical Deep Dives |  |
| 143 | Negotiation Research |  |
| 144 | Active Interviewing 5 |  |
| 145 | Final Round Prep 1 |  |
| 146 | Practice Final Rounds |  |
| 147 | Month 5 Review |  |

---

## MONTH 6: CLOSE OFFERS

### Week 22: Final Push

| Day | Topic | Status |
|-----|-------|--------|
| 148 | Final Round Prep 2 |  |
| 149 | Final Interviews 1 |  |
| 150 | Final Interviews 2 |  |
| 151 | Compare Offers |  |
| 152 | Negotiation Strategy |  |
| 153 | Negotiate Aggressively |  |
| 154 | Review Offer Details |  |

### Week 23: Negotiate + Accept

| Day | Topic | Status |
|-----|-------|--------|
| 155 | Counter Offer Calls |  |
| 156 | Final Negotiations |  |
| 157 | Accept Offer |  |
| 158 | Give Notice |  |
| 159 | Transition Planning |  |
| 160 | Celebrate |  |
| 161 | Prep New Role |  |

### Week 24: Launch

| Day | Topic | Status |
|-----|-------|--------|
| 162 | Knowledge Transfer |  |
| 163 | Finish Strong |  |
| 164 | Study New Stack |  |
| 165 | Prep Day 1 |  |
| 166 | Last Day |  |
| 167 | Rest + Celebrate |  |
| 168 | New Chapter Begins |  |

---
